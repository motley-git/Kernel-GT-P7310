...
{
	struct regulator *vdd_ldo4_regulator;

	vdd_ldo4_regulator = regulator_get(NULL, "vdd_ldo4");
	if (IS_ERR(vdd_ldo4_regulator)) {
		pr_err("%s: failed to get %s\n", __func__, "vdd_ldo4");
		return (-ENODEV);
	}
	regulator_force_disable(vdd_ldo4_regulator);
	regulator_put(vdd_ldo4_regulator);
